...
const DailyDueCollection = () => {
    ...
    const totalPaid = calculateTotalPrice(dueCollectionSaleData?.result?.result?.map(sale => Number(sale?.todayPaid)))
    ...
    return (
        <div className={dueCollection.main}>
            <div className={`${dueCollection.title} flex_left`}>
                <i onClick={() => {
                    dispatch(openModal('daily-due-collection'))
                    dispatch(addSalesData({modifiedData:modifiedProductDataWithIndexId, totalSalesValue, totalSalesItem, totalDiscount, totalPaid}))
