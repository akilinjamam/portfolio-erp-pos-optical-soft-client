    ...
    return (
        <div className={`${imgmodal.main} flex_center  ${(open && type === 'today-due-collection' ) ? imgmodal.open : imgmodal.close}`} >
                <section className={`${imgmodal.container} ${imgmodal.sizeTodaySales}`}>
                    ...
                    <TodayDueCollectionTable contentToPrint={contentToPrint} paginatedDataContainer={saleData} totalSalesValue={totalSalesValue} totalSalesItem={totalSalesItem} totalTodayPaid={totalPaid} totalDiscount={totalDiscount}/>
                    </div>        
                </section>
        </div>
    );
