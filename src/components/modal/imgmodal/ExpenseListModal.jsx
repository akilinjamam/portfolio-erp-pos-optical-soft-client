...
import VendorListTable from '../../dashboard/accounts_module/DailyCashExpensesList/DailyCashExpensesTable';


const ExpenseListModal = ({type, open, dispatch, closeModal, expenseData}) => {
    ...
    return (
        <div className={`${imgmodal.main} flex_center  ${(open && type === 'expense' ) ? imgmodal.open : imgmodal.close}`} >
                <section className={`${imgmodal.container} ${imgmodal.sizeStock}`}>
                    <div className={`${imgmodal.cancelBtn} flex_between`}>
                        <div>
                        <button>Total payroll Data : {expenseData?.length} </button>
                        <button 
                        onClick={() => {
